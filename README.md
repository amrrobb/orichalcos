...
- **v3.5 — idle capital earns.** Between epochs the pool's USDC sits at 0% APY — that's strictly dominated. v3.5 routes the surplus above a contract-enforced liquidity buffer (`reserved ≥ Σ open maxClaim`) into `MockYieldVault` (already deployed at `0x5c16…2fE1`, currently unwired). On `settleClaim` / LP `withdraw` the pool auto-pulls from the vault first if cash is short. LPs earn the 8% APR floor on idle capital **in addition to** the existing premium yield — same risk surface, strictly more yield. *Minimum demo:* `setLiquidityBufferBps(5000)` keeper call routes 50% of free pool to vault; a subsequent `markBreach` + `settleEpoch` proves the auto-pull works under settlement pressure. Real MockUSDC, real on-chain flows. The contract surface is ~80 lines (a sweeper + auto-pull on settle/withdraw); deferred from this submission because it's a contract change and v3's five addresses are the verified demo surface.
