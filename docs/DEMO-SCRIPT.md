...
Each shot has one **"Caption / narration"** paragraph — natural sentences, conversational length. Read it aloud as written. Per-word caption breaks will be auto-generated later when you run the video through a captioner (YouTube Studio, CapCut, Descript, etc.) — those tools cut the paragraph into 3-5 word lines automatically based on your speech timing.

You don't need a separate SRT file. The script paragraphs are the caption source.
...
**Caption / narration:**
> Every AI trader sits on one of two horns. Reveal the strategy and alpha decays. Hide it and no challenger can trust it. There has been no primitive for verifiable performance without revealed alpha — until now.
...
**Caption / narration:**
> Three roles, one protocol. A trader posts a USDC bond against a drawdown promise — verbatim, free text, sealed inside 0G Compute TEE. A challenger places a stake against the promise. An LP underwrites the float. Three primitives, one verifiable trail.
...
**Caption / narration:**
> Six steps from bond to settlement. Trader bonds. Promise gets sealed in TEE. Trades fill on Hyperliquid testnet. Challengers stake. Breach or kept-promise. Permissionless settlement. On breach the bond pays the challenger. On a kept promise the stake splits sixty-forty — trader earns yield for being right, LP earns the rest. No oracle, no admin. Let me show you live on chain.
...
**Caption / narration:**
> This is the live market on 0G mainnet, chain ID sixteen-six-sixty-one. Every card is a wager — a trader who's posted a bond against a drawdown promise, and the headline reads the promise verbatim. The protocol is permissionless: anyone mints a wager, anyone stakes against one, anyone settles.
...
**Caption / narration:**
> A trader writes their own promise — free text, their own words. When they hit mint, we encrypt the promise to 0G Storage, run a real TEE inference inside Intel TDX on 0G Compute, verify the chatId via processResponse, and commit the result to an INFT on 0G Chain. Four 0G components, end-to-end.
...
**Caption / narration:**
> And here it is on 0G mainnet — the StrategyINFT contract, chainId sixteen-six-sixty-one. Every wager you just saw on the grid is bonded to this contract. The most recent mints carry real TEE-verified chatIds and real 0G Storage merkle roots, all on chain.
...
**Caption / narration:**
> Now the challenger side. Pick a claim size — five hundred USDC. The stake is twelve and a half percent up front. Two outcomes pre-computed and shown side by side: if the trader breaks the promise, you claim from the bond, net plus four hundred thirty-seven. If they keep it, your stake pays the trader for being right.
...
**Caption / narration:**
> And this is the trust layer. Every trade carries a TEE attestation chatId from 0G Compute, a 0G Storage merkle root for the reasoning bundle, and the actual Hyperliquid testnet transaction hash. Click the Hyperliquid link, you land on the real L1 fill. No off-chain trust.
...
**Caption / narration:**
> And here's the proof on chain — a wager that kept its promise. The settlement transaction emits PolicyExpired with premium to LP two point five USDC, premium to trader three point seven five USDC. That's the sixty-forty split, settled on chain by any wallet calling settleEpoch. Bond, stake, settlement — all verifiable. Orichalcos, built for Track Two on 0G mainnet.
